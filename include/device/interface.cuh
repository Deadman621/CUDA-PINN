...
namespace device::tensor {
    ...
    class interface: public variables<T> {
        private:
            ...
            void copy_to_x(T* x, cudaMemcpyKind kind) const {
                if (this->n > 0) {
                    ...
                    if (!data_allocated)
                        throw std::invalid_argument{"device::copy_to: copying to unallocated raw data (x)"};
        ...
        public:
            ...
            data_allocated{obj.data_allocated}, shape_allocated{obj.shape_allocated} {
                this->data = obj.data;
                this->n = obj.n;
                this->dim = obj.dim;
            ...
            interface& copy_from(const T* x, cudaMemcpyKind kind) {
                if (this->n > 0) {
                    ...
                    if (!data_allocated)
                        throw std::invalid_argument{"device::copy_from: copying to unallocated raw data (x)"};
